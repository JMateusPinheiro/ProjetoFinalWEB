...
<body>
	...
	<div class="col-lg-6">
		<form action="/adm/gerenciar_servicos/${path}" method="post">
			<img src="/img/Logos/1_Primary_logo_on_transparent.png"
				class="img-responsive" alt="Logo" />
			<c:if test="${servico != null }">
				<input type="hidden" name="id" value="${servico.id}" />
				<h1 class="center-text">Editar Serviço</h1>
			</c:if>
			<c:if test="${servico == null }">
				<h1 class="center-text">Criar Serviço</h1>
			</c:if>
			<div class="form-group">
				<input type="text" name="nome" placeholder="Nome"
					class="form-control" value="${servico.nome }" required />
			</div>
			<div class="form-group">
				<input type="number" class="form-control" placeholder="Preço"
					name="preco" step="0.01" value="${servico.preco }" required />
			</div>
			<div class="form-group">
				<input type="text" name="descricao" placeholder="Descrição"
					class="form-control" value="${servico.descricao}" required />
			</div>
